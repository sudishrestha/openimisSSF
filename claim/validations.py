...
REJECTION_REASON_INVALIDATED_IN_SOSYS = 20
...
def validate_item_product_family(claimitem, target_date, item, insuree_id, adult,claim):
    errors = []
    found = False
    with get_products(target_date, item.id, insuree_id, adult, 'Item',claim) as cursor:
...
def validate_service_product_family(claimservice, target_date, service, insuree_id, adult, base_category,claim=None):
    errors = []
    found = False
    with get_products(target_date, service.id, insuree_id, adult, 'Service',claim) as cursor:
...
def get_products(target_date, elt_id, insuree_id, adult, item_or_service,claim=None):
    ...
    sql = f"""
        ...
        WHERE (tblInsuree.ValidityTo IS NULL) AND (tblInsuree.InsureeId = %s)
            ...
            AND tblPolicy.ProdID ={claim.product.id if claim.product else ''} 
        ORDER BY DATEADD(m,ISNULL(tblProduct{item_or_service}s.{waiting_period}, 0),
            tblPolicy.EffectiveDate)            
    """
    print('Claim validation product'+item_or_service,claim.product.id)
    cursor.execute(sql,
...
def validate_assign_prod_elt(claim, elt, elt_ref, elt_qs):
    ...
    elt.product_id = claim.product.id
    ...
    #     ).first()
    elt.policy = Policy.objects.filter(
        family_id=claim.insuree.family_id,
...
def process_dedrem(claim, audit_user_id=-1, is_process=False):
    ...
    for policy_product in items_query.union(services_query, all=True):
        print('policy_product',policy_product)
        product = Product.objects.get(id=policy_product["product_id"])
        print('proooooduct',product)
    ...
    if (ceiling_interpretation == "I" and hospitalization == 1) or (ceiling_interpretation == "H" and hf_level == "H"):
        ...
        claim_ded_rem_to_create["rem_ip"] = claim.claimed
    else:
        claim_ded_rem_to_create["ded_op"] = deducted
        #claim_ded_rem_to_create["rem_op"] = remunerated
        claim_ded_rem_to_create["rem_op"] = claim.claimed
