...
from api_fhir_r4.serializers import PatientSerializer, LocationSerializer, LocationSiteSerializer, PractitionerRoleSerializer, \
    PractitionerSerializer, ClaimSerializer, CoverageEligibilityRequestSerializer,EmployerServiceSerializer, EmployeeeServiceSerializer,\
...
class ClaimViewSet(BaseFHIRView, mixins.RetrieveModelMixin, mixins.ListModelMixin,
    ...
    def list(self, request, *args, **kwargs):
        ...
        if identifier is not None:
            ...
            queryset = queryset.filter(validity_to__isnull=True, employer=employer,status=4).order_by('validity_from')
        else:
            queryset = queryset.filter(validity_to__isnull=True).order_by('validity_from')
...
class EmployerServiceViewSet(BaseFHIRView, mixins.RetrieveModelMixin, mixins.ListModelMixin, GenericViewSet):
    lookup_field = 'E_SSID'
    serializer_class = EmployerServiceSerializer
    #permission_classes = (FHIRApiHealthServicePermissions,)

    def get_queryset(self):
        #return HealthFacility.get_queryset(None, self.request.user)
        return Employer.get_queryset(None, self.request.user)

class EmployeeServiceViewSet(BaseFHIRView, mixins.RetrieveModelMixin, mixins.ListModelMixin, GenericViewSet):
    lookup_field = 'uuid'
    serializer_class = EmployeeeServiceSerializer
    #permission_classes = (FHIRApiHealthServicePermissions,)
